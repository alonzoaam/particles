...
*/
    ...
    // Fluid, go home, curl, go home, brownian motion.
    this.times = [15, 20, 25, 35, 40, 45, 50];
    this.ops = [this.fluid_set, this.return_to_home, this.curl_set, this.return_to_home, this.brownian_motion_set, this.curl_set, this.return_to_home];
    this.hasRun = false;
    this.hasRun2 = false;
  }
...
export class Project extends project_base {
  render_animation(caller) {  // display():  Called once per frame of animation.
    ...
    while (this.t_sim < t_next) {
      ...
      this.hasRun = false;
      // Change this.simulate to use an array of functions to call.
      ...
      if (this.simulate) {
        ...
        console.log("the rel_time is "+ rel_time);
        console.log("The index is "+this.index);
        const bound_op = op.bind(this);
        if(rel_time < time){
          bound_op();
        }
        else{
        ...
        /*if (rel_time < this.start_fluid) {
          this.fluid = true;
            ...
            (rel_time >= this.start_fluid && rel_time < this.start_go_home) || rel_time >= this.start_go_home2) { // go home
          this.reset_vals();
        ...
        } else if (rel_time >= this.start_go_home && rel_time < this.start_go_home2) { // curl
          //this.reset_vals();
          this.returnHome = false;
          if (!this.hasRun2) {
            for (let q = 0; q < this.canvas_particles.length; q++) {
